...
export function DriverDashboard({ navigation }) {
  ...
  if (isDesktopWeb) {
    const pendingRequests = bookingRequests.filter(r => !r.responded);
    return (
          ...
          <View style={{
            ...
            <View>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 30 }}>
                ...
                <View>
                  ...
                  <Text style={{ fontSize: 11, color: colors.textSub }}>Driver Console</Text>
                </View>
              </View>
              ...
              <View style={{ gap: 8 }}>
                {[
                  { id: 'home', label: 'My Dashboard', icon: '🏠', route: null },
                  { id: 'trips', label: 'Dispatch Trips', icon: '📦', route: 'MyTrips' },
                  { id: 'jobs', label: 'Available Jobs', icon: '🔔', route: 'Jobs' },
                  { id: 'earn', label: 'Earnings & Wallet', icon: '💰', route: 'Earnings' },
                  { id: 'vehicle', label: 'Vehicle Specs', icon: '🚛', route: 'Vehicle' },
                  { id: 'profile', label: 'Driver Profile', icon: '👤', route: 'DriverProfileSetup' }
                ].map(item => {
            ...
            <View style={{ borderTopWidth: 1.5, borderTopColor: colors.border, paddingTop: 16 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 13, fontWeight: '800', color: colors.text }} numberOfLines={1}>{driver?.name || 'Driver'}</Text>
                  <Text style={{ fontSize: 11, color: colors.textSub }} numberOfLines={1}>{driver?.city || 'Chennai Hub'}</Text>
                </View>
              </View>
              <TouchableOpacity
            ...
            {activeTab === 'home' ? (
              <View style={{ maxWidth: 1000, alignSelf: 'center', width: '100%' }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                  <View>
                    <Text style={{ fontSize: 14, color: colors.textSub }}>Welcome Back!</Text>
                    <Text style={{ fontSize: 28, fontWeight: '900', color: colors.text }}>{driver?.name || 'Driver'} Console</Text>
                  </View>
                </View>
                ...
                <View style={{ flexDirection: 'row', gap: 20 }}>
                  {/* Left Column: Requests & Vehicle */}
                  <View style={{ flex: 1.2, gap: 20 }}>
                    {pendingRequests.length > 0 && (
                      <View style={{ backgroundColor: colors.orange + '15', borderRadius: 14, borderWidth: 1.5, borderColor: colors.orange + '40', padding: 12 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                          <Text style={{ fontSize: 20 }}>🔔</Text>
                          <Text style={{ fontSize: fonts.base, fontWeight: '800', color: colors.orange, flex: 1 }}>
                            {pendingRequests.length} New Booking Request{pendingRequests.length > 1 ? 's' : ''}!
                          </Text>
                        </View>
                        {pendingRequests.slice(0, 2).map((req, i) => (
                          <View key={req.id || i} style={{ backgroundColor: colors.surface, borderRadius: 10, padding: 14, marginBottom: 10 }}>
                      ...
                      {driver?.vehicle ? (
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14 }}>
                          ...
                          <View>
                            ...
                            <Text style={{ fontSize: fonts.sm, color: colors.textMuted }}>License: {driver.license || 'Not added'}</Text>
                          </View>
                        </View>
                      ) : (
                  ...
                  <View style={{ flex: 1, gap: 20 }}>
                    ...
                    <Card style={{ padding: 20, gap: 10 }}>
                      <Btn label="📋 My Trips Ledger" onPress={() => navigation.navigate('MyTrips')} style={{ marginBottom: 0 }} />
                      <Btn label="🔔 Open Cargo Jobs" onPress={() => navigation.navigate('Jobs')} variant="outline" style={{ marginBottom: 0 }} />
                      <Btn label="💰 Driver Wallet" onPress={() => navigation.navigate('Earnings')} variant="outline" style={{ marginBottom: 0 }} />
                    </Card>
                  </View>
                </View>
              </View>
