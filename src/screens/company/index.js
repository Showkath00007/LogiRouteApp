          ...
          <View style={{
            ...
            <View>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 30 }}>
                ...
                <View>
                  <Text style={{ fontSize: 18, fontWeight: '900', color: colors.text }}>LogiRoute</Text>
                  <Text style={{ fontSize: 11, color: colors.sub }}>Enterprise Portal</Text>
                </View>
              </View>
              ...
              <View style={{ gap: 8 }}>
                {[
                  { id: 'home', label: 'Dashboard Hub', icon: '🏠', route: null },
                  { id: 'shipments', label: 'Cargo Shipments', icon: '📦', route: 'Shipments' },
                  { id: 'optimize', label: 'Route Optimizer', icon: '⚡', route: 'Optimizer' },
                  { id: 'analytics', label: 'Tolls & Reports', icon: '📊', route: 'Reports' },
                  { id: 'payouts', label: 'Payout Release Hub', icon: '💸', route: 'PayoutRelease' },
                  { id: 'weather', label: 'Road Alerts Radar', icon: '🛣️', route: 'RoadAlerts' },
                  { id: 'profile', label: 'Settings & Profile', icon: '👤', route: 'Profile' }
                ].map(item => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text style={{ fontSize: 13, fontWeight: isActive ? '800' : '600', color: isActive ? colors.accent : colors.textSub }}>
                        {item.label}
                      </Text>
                    </TouchableOpacity>
                  );
            ...
            {/* User Profile Footer */}
            <View style={{ borderTopWidth: 1.5, borderTopColor: colors.border, paddingTop: 16 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                <View style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: colors.accent, overflow: 'hidden', alignItems: 'center', justifyContent: 'center' }}>
                  {profile?.avatar ? (
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 13, fontWeight: '800', color: colors.text }} numberOfLines={1}>{profile?.name || 'Kadiyala Logistics'}</Text>
                  <Text style={{ fontSize: 11, color: colors.textMuted }} numberOfLines={1}>{profile?.city || 'Chennai Hub'}</Text>
                </View>
              </View>
              <TouchableOpacity
              ...
              >
                ...
                <Text style={{ fontSize: 12, fontWeight: '700', color: colors.red }}>Sign Out</Text>
              </TouchableOpacity>
            </View>
          </View>

          {/* Desktop Right Main Content Panel */}
          <ScrollView contentContainerStyle={{ padding: 30, flexGrow: 1 }} style={{ flex: 1 }}>
            {/* Main dashboard grid or tabs render */}
            {activeTab === 'home' ? (
              <View style={{ maxWidth: 1000, alignSelf: 'center', width: '100%' }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                  <View>
                    <Text style={{ fontSize: 14, color: colors.sub }}>{t('goodMorning')}</Text>
                    <Text style={{ fontSize: 28, fontWeight: '900', color: colors.text }}>Kadiyala Logistics Portal</Text>
                  </View>
                </View>

                {/* Grid stats display */}
                <View style={{ flexDirection: 'row', gap: 16, marginBottom: 20 }}>
                  <StatCard icon="📦" value={String(activeCount)} label={t('activeShipments')} color={colors.blue} style={{ flex: 1 }} />
                  <StatCard icon="⏱" value="—" label="On Time" color={colors.accent} style={{ flex: 1 }} />
                  <StatCard icon="💰" value={thisMonthLabel} label={t('minCost')} color={colors.green} style={{ flex: 1 }} />
                  <TouchableOpacity onPress={() => navigation.navigate('Notifications')} style={{ flex: 1 }}>
                ...
                <View style={{ flexDirection: 'row', gap: 20 }}>
                  {/* Left Column: Live Tracker & Sustainability */}
                  <View style={{ flex: 1.2, gap: 20 }}>
                    <SectionLabel label="Live Fleet & Route Visualizer" />
                    {!activeShipment ? (
                      <Card style={{ padding: 24, alignItems: 'center', justifyContent: 'center', borderStyle: 'dashed', borderWidth: 1.5, borderColor: colors.border }}>
                        <Text style={{ fontSize: 32, marginBottom: 8 }}>🚚</Text>
                        <Text style={{ fontSize: 14, fontWeight: '700', color: colors.text, marginBottom: 4 }}>No Active Cargo in Transit</Text>
                        <Text style={{ fontSize: 12, color: colors.textSub, textAlign: 'center', marginBottom: 12 }}>
                          Create a new shipment or mark one as "In Transit" to view its live path animation.
                        </Text>
                        <Btn label="Simulate Transit Cargo 🧪" onPress={() => setSimulatedCargo(true)} variant="outline" style={{ marginBottom: 0 }} />
                      </Card>
                    ) : (
                      <Card style={{ padding: 20 }}>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                          <View>
                            <Text style={{ fontSize: 15, fontWeight: '850', color: colors.text }}>
                          ...
                          <View style={{ alignItems: 'flex-end', gap: 6 }}>
                            <Badge label="IN TRANSIT" type="green" />
                        ...
                        {/* Animated Route Line */}
                        <View style={{ height: 40, justifyContent: 'center', marginVertical: 12, position: 'relative' }}>
                        ...
                        {/* Telemetry info */}
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', borderTopWidth: 1.5, borderColor: colors.border, paddingTop: 12, marginTop: 12 }}>
                          {[['SPEED', '64 km/h'], ['ETA', '2.5 hrs'], ['WEATHER', 'Clear ☀️'], ['CARGO', activeShipment.material]].map(([k, v]) => (
                            <View key={k} style={{ alignItems: 'center' }}>
                              <Text style={{ fontSize: 10, color: colors.textMuted }}>{k}</Text>
                              <Text style={{ fontSize: 13, fontWeight: '800', color: k === 'WEATHER' ? colors.green : colors.text }}>{v}</Text>
                            </View>
                          ))}
                        </View>
                      </Card>
                  ...
                  {/* Right Column: Actions & Recent Shipments */}
                  <View style={{ flex: 1, gap: 20 }}>
                    <SectionLabel label="Operations Hub" />
                    <Card style={{ padding: 20, gap: 10 }}>
                      <Btn label={t('newShipment')} onPress={() => navigation.navigate('NewShipment')} style={{ marginBottom: 0 }} />
                      <Btn label={t('optimizeRoute')} onPress={() => navigation.navigate('Optimizer')} variant="outline" style={{ marginBottom: 0 }} />
                      <Btn label="📢 Post Job for Any Driver" onPress={() => navigation.navigate('PostJob')} variant="outline" style={{ marginBottom: 0 }} />
                    </Card>
                    ...
                    {loading ? (
                      ...
                      <Card>
                        <Text style={{ textAlign: 'center', color: colors.sub, paddingVertical: 20 }}>No shipments yet.</Text>
                      </Card>
                    ) : (
                          ...
                          <Card style={{ marginBottom: 10 }}>
                            <View style={{ flexDirection: 'row', gap: 10, alignItems: 'flex-start' }}>
                              ...
                              <View style={{ flex: 1 }}>
                                ...
                                <Text style={{ fontSize: 12, color: colors.accent, fontWeight: '750', marginTop: 4 }}>₹{s.cost?.toLocaleString()}</Text>
                              </View>
                            </View>
                          </Card>
                        </TouchableOpacity>
